...
/**
 * Servlet implementation class UpdateGetContoller
 */
@WebServlet("/UpdateGetContoller")
public class UpdateGetContoller extends HttpServlet {
...
//       }
    ...
    try {			
		...
		PreparedStatement ps=con.prepareStatement("UPDATE product SET pname = ?,price = ?, weight = ?, details = ? WHERE  id= ?");
  	 ...
  	 int y=0;
  	   ...
  	   if(y!=0)
  	    {	RequestDispatcher rd=request.getRequestDispatcher("ViewProductContoller");
  	    	request.setAttribute("update","Product succuessfully Updated");
